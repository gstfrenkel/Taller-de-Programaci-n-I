...
use super::{compact_size::CompactSizeUInt, message_error::MessageError, header::MessageHeader, message_constants::INV_COMMAND};
...
impl Inv {
    ...
    pub fn from_bytes(header: MessageHeader, stream: &mut dyn Read) -> Result<Inv, MessageError> {
        ...
        let data_type = inventory_list.last().ok_or(MessageError::ReadFromBytes)?.get_type();

        Ok(Inv{ _header: header, _count: count, inventory_list, data_type })
    }
