...
fn filter_mempool(utxo: &Arc<Mutex<UnspentTx>>, unconfirmed_txs: &[WalletTx]) -> Result<Vec<(TxOut, i64)>, TxFilterError> {
    let locked_utxo = utxo.lock().map_err(|_| TxFilterError::LockUtxo)?;
    let mut used_txouts = vec![];
...
pub fn get_wallet_txns(blockchain: &Arc<Mutex<BlockChain>>, utxo: &Arc<Mutex<UnspentTx>>, mempool: &Arc<Mutex<Mempool>>, get_transactions: GetTransactions) -> Result<Transactions,TxFilterError>{
    ...
    let (unconfirmed_txs_send, unconfirmed_txs_recv, mut used_txouts) = filter_unconfirmed_transactions(mempool, pk_script, public_key.clone())?;
    ...
    Ok(Transactions::new(confirmed_txs_send, confirmed_txs_recv, unconfirmed_txs_send, unconfirmed_txs_recv, utxo_txs, used_txouts, last_update))
}
