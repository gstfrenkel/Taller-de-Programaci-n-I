...
pub fn headers_download(
    ...
    let mut file = OpenOptions::new()
        ...
        .read(true)
        .append(true)
        .open(HEADERS_FILE_PATH).map_err(|_| NetworkError::HeaderDownload)?;

    let mut header_list: Vec<BlockHeader> = load_headers(&mut file)?;

    println!(
        "Se contaba con {} headers previamente descargados",
        header_list.len()
    );
...
mod test_header_download {
    ...
    fn test_save_genesis_in_file() -> Result<(), NetworkError> {
        ...
        file.seek(std::io::SeekFrom::Start(0))
            .expect("Error while seeking to the start of the file");
