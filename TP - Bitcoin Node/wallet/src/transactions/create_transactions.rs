...
pub fn address_from_pubkey(
    public_key: &[u8],
    p2wpkh: bool,
) -> Result<Vec<u8>, TransactionCreateError> {
    let h160 = hash160::Hash::hash(public_key).to_byte_array();

    if p2wpkh {
        let witness_program = WitnessProgram::new(h160.to_vec())?;

        return Ok(witness_program.to_address()?.as_bytes().to_vec());
    }
...
pub fn pk_script_from_pubkey(
    public_key: &[u8],
    p2wpkh: bool,
) -> Result<Vec<u8>, TransactionCreateError> {
    let address = address_from_pubkey(public_key, p2wpkh)?;

    Ok(pk_script_from_address(&address, p2wpkh))
}
...
pub fn pk_script_from_address(address: &Vec<u8>, p2wpkh: bool) -> Vec<u8> {
    if p2wpkh {
        ...
        if let Ok(witness_program) =
            WitnessProgram::from_address(/*"tb".to_string(),*/ string_address)
        {
...
fn was_txout_used(txout: TxOut, used_txouts: &[TxOut]) -> bool {
    for used_txout in used_txouts {
        if txout.to_bytes() == used_txout.to_bytes() {
            return true;
        }
    }
...
fn create_txin_list(
    ...
    total_amount: i64,
    used_txouts: &[TxOut],
) -> Result<(Vec<TxIn>, Vec<i64>), TransactionCreateError> {
...
fn sign_transaction(
    ...
    for i in 0..transaction.get_tx_in_list().len() {
        if p2wpkh {
            let signature_hash =
                transaction.p2wpkh_signature_hash(i, pk_script.to_vec(), amount_list.to_vec());
            ...
            transaction.set_signature(i, script.to_bytes());
...
mod create_transactions_test {
    ...
    use crate::transactions::{
        create_transaction_error::TransactionCreateError,
        create_transactions::{
            address_from_pubkey, decode_base58, is_array_bech32, is_string_bech32, sign_transaction,
        },
    };
    ...
    pub fn create_transaction() -> Result<(), TransactionCreateError> {
        ...
        let change_txout = TxOut::new(change_amount as i64, change_script.to_bytes());
        ...
        let target_txout = TxOut::new(target_amount as i64, target_script.to_bytes());

        let mut tx = Transaction::new(1, vec![txin], vec![change_txout, target_txout], 0, false);

        let secp = Secp256k1::new();

        let signature_hash = tx.p2pkh_signature_hash(0, &change_script.to_bytes());
        ...
        tx.set_signature(0, signature_script.to_bytes());

        println!("{:?}", encode_hex(&tx.to_bytes(false))?);
    ...
    pub fn test_create_transaction_2() -> Result<(), TransactionCreateError> {
        ...
        println!("len script {}", signature_script.to_bytes().len());
    ...
    pub fn test_is_bech32() {
        assert!(is_string_bech32(
            "tb1qw508d6qejxtdg4y5r3zarvary0c5xw7kxpjzsx".to_string()
        ));
        assert!(!is_string_bech32(
            "tb1qw308d6qejxtdg4y5r3zarvary0c5xw7kxpjzsx".to_string()
        ));
    }
    ...
    pub fn test_pk_script() {
        ...
        assert_eq!(
            pk_script_from_address(&address.as_bytes().to_vec(), true),
            [
                0, 20, 117, 30, 118, 232, 25, 145, 150, 212, 84, 148, 28, 69, 209, 179, 163, 35,
                241, 67, 59, 214
            ]
        );
        assert_eq!(
            pk_script_from_address(&address2, true),
            pk_script_from_address(&address2, false)
        );
    }
    ...
    pub fn test_pk_to_address() -> Result<(), TransactionCreateError> {
        ...
        let public_key2 =
            decode_hex("FEABBD73DDD97F2C00CC6023B38C08214736CAF26A8ED91CE1ABA30D8BE46B35")?;
    ...
    pub fn test_signature() -> Result<(), TransactionCreateError> {
        let h160: Vec<u8> = vec![
            0, 20, 117, 30, 118, 232, 25, 145, 150, 212, 84, 148, 28, 69, 209, 179, 163, 35, 241,
            67, 59, 214,
        ];
        let private_key =
            decode_hex("11063638E1C47A9EEEDCDB476654644B00F7BFF9798031CFBB1EB9DA4D8B51F4")?;
        let private_key =
            SecretKey::from_slice(&private_key).map_err(|_| TransactionCreateError::PrivateKey)?;
    ...
    pub fn test_p2wpkh_tx() -> Result<(), TransactionCreateError> {
        ...
        let private_key = SecretKey::from_slice(&decode_hex(
            "619c335025c7f4012e556c2a58b2506e30b8511b53ade95ea316fd8c3286feb9",
        )?)
        .map_err(|_| TransactionCreateError::PrivateKey)?;

        let mut cursor = Cursor::new(tx_hex);
        let mut tx = Transaction::from_bytes(&mut cursor)?;
        ...
        let final_tx = Transaction::from_bytes(&mut cursor)?;

        assert_eq!(
            final_tx.get_witness()[1].stack_items[0],
            tx.get_witness()[1].stack_items[0]
        );
        assert_eq!(
            final_tx.get_witness()[1].stack_items[1],
            tx.get_witness()[1].stack_items[1]
        );
