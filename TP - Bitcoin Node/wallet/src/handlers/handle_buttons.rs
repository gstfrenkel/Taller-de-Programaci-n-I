...
use node::messages::read_from_bytes::{decode_hex, read_string_from_bytes, encode_hex};
...
fn set_make_proof_button(builder: &Builder, node: Arc<Mutex<TcpStream>>) -> Result<(), InterfaceError> {
    ...
    make_proof_button.connect_clicked(move |_| {
        ...
        if proof_of_inclusion {
            ...
            poi_advice_label.set_text("The requested transaction was successfully verified as part of the block.");
            poi_error_image.hide();
        }
        else {
            poi_title_label.set_text("Unsuccessful Proof of Inclusion");
            poi_advice_label.set_text("The requested transaction was not verified as part of the block.");
            poi_success_image.hide();
        }
    ...
    send_transaction_button.connect_clicked( move |_| {
        let target_list = match get_target_list(&transaction_box){
            ...
            Err(InterfaceError::MissingAddress) => {
                let err_text = "Please, fill in all of the recipient's address.";
                title_label.set_text("Missing Address");
                println!("{}", err_text);
                user_authentication_dialog.show_all();
                advice_label.set_text(err_text);
                return;
            }
            Err(InterfaceError::MissingAmount) => {
                let err_text = "Please, fill in all of the transaction amounts.";
                title_label.set_text("Missing Amount");
                println!("{}", err_text);
                user_authentication_dialog.show_all();
                advice_label.set_text(err_text);
                return;
            ...
            if let Some(user_info) = locked_accounts.get_current_account_info(){
                ...
                match create_transaction(target_list, user_info.get_utxo(), private_key, fee as i64, &used_txouts, user_info.get_bech32()){
                    ...
                    Err(TransactionCreateError::InsufficientFunds) => {
                        let err_text = "There are not enough funds in the account to create this transaction.";
                        title_label.set_text("Unavailable Outputs");
                        println!("{}", err_text);
                        user_authentication_dialog.show_all();
                        advice_label.set_text(err_text);
                    },
                    Err(TransactionCreateError::UnavailableOutput) => {
                        let err_text = "There are not enough confirmed outputs in the account to create this transaction. Please try again later.";
                        println!("{}", err_text);
                        title_label.set_text("Unavailable Outputs");
                        user_authentication_dialog.show_all();
                        advice_label.set_text(err_text);
